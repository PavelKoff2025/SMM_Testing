...
# Демо-автосид: при DEMO_AUTOSEED=1 и пустой БД старт автоматически заливает
# демо-тест лекции 1 и двух студентов с попытками — чтобы на эфемерном
# диске Render free комиссия сразу видела живой сервис (тесты, аналитику,
# разбор). Локально флаг выключен — локальная БД не трогается.
DEMO_AUTOSEED = os.getenv("DEMO_AUTOSEED", "0") == "1"
